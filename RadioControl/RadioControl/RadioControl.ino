...
const bool DEBUG = false; // verbose debugging, turn this off for faster execution time
...
void waitInputByte(int timeout)
{
  if (DEBUG) {PC.println(F("waiting for radio packet..."));}
  ...
  do
  {
    ...
    {
      if (DEBUG) {PC.print(millis());}
      if (DEBUG) {PC.println(F(" packet found"));}
      // we do have a valid packet
...
void waitInputPWM(int timeout)
{
  ...
  do
  {
    ...
    {
      // TODO need to check which interrupts are actually used, may need more ISRs
    }
    if (timeout > 0)
    {
      delay(1);
...
void loopReceiver()
{
  if (DEBUG) {PC.println(F("start loop"));}
  int timeout = 100;
  inputData.newData = false;
  if (using_us_units)
  {
...
void sendElcanoSerial()
{
  ...
  if (DEBUG) {PC.print(millis());}
  if (DEBUG) {PC.println(F(" sent ES packet: "));}
#endif
}
