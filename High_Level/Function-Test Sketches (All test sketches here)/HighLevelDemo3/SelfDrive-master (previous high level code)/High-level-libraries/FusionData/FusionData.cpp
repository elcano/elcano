...
void PositionData::Display()
{
	...
	//Serial.print("PositionData::speed_mmPs->");
	//Serial.println(speed_mmPs);
...
{
	oldData.x_Pos_mm = newData.x_Pos_mm;
	oldData.y_Pos_mm = newData.y_Pos_mm;
	oldData.bearing_deg = newData.bearing_deg;
	oldData.time_ms = newData.time_ms;
	oldData.speed_mmPs = newData.speed_mmPs;
...
void TranslateCoordinates(PositionData &in, PositionData &out, int shift)
{
	if (shift == 1)
	{
		...
		out.x_Pos_mm = out.x_Pos_mm - in.x_Pos_mm;
		out.y_Pos_mm = out.y_Pos_mm - in.y_Pos_mm;
	}
	else
	{
...
{
	if (x2 == x1)
		return(y2>y1? MAX_DISTANCE: -MAX_DISTANCE); //infinity
	
	double slope = ((y2-y1) / (x2 - x1));
	
	if(slope > MAX_DISTANCE)
		return MAX_DISTANCE;
...
//	//Serial.println(distance_mm);
//	double DE = distance_mm * 100*DISTANCE_ERROR;
...
//
//	long gx0 = gps.x_Pos_mm/10000; //why divid by 10000
//
//	// Slope Selection
...
//
//	// Get Cross Point X
//	out.x_Pos_mm = CrossPointX(m1, c1, m2, c2);
...
//	// sy0 = 0
//	long sy0 = 0;   //should this be 0 ???
//	long gy0 = gps.y_Pos_mm/10000; //why divid by 10000
//
//	if (sy0 <= gy0)
//	{
...
//	//Serial.println(c2);
//	// Get CrossPoint Y
//	out.y_Pos_mm = CrossPointY(m1, c1, m2, c2);
