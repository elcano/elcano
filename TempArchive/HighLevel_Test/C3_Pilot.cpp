...
String wrdState[] = { "STOP", "STRAIGHT", "ENTER_TURN", 
    "LEAVE_TURN", "APPROACH_GOAL", "LEAVE_GOAL", "STARTING" }; //used for clearer debugging
 ...
 * param
 *****************************************************************************************************/
bool proper_heading(int n) {
  long northDiff = abs(estPos.north_mm - pathz[n].north_mm);
  if(estPos.north_mm > pathz[n].north_mm) 
    northDiff = northDiff * -1; //heading south
  
  long eastDiff = abs(estPos.east_mm - pathz[n].east_mm);
  if(estPos.east_mm > pathz[n].east_mm)
    eastDiff = eastDiff * -1; //heading west
  
  
  double properHeading = atan2(northDiff, eastDiff) * 180.0 / PI;
  Serial.println("Heading needed: " + String(properHeading) + ", Current Heading: " + String(estPos.bearing_deg));

  //TODO fix 360 bug
  if(estPos.bearing_deg >= (properHeading - 5) && estPos.bearing_deg <= (properHeading + 5)) {
    Serial.println("Within 5 Degrees");
  ...
  if(properHeading > estPos.bearing_deg) {
    if(estPos.bearing_deg + 180 >= 360 || estPos.bearing_deg >= properHeading) { //right turn
  ...
  if(properHeading < estPos.bearing_deg) {
    if(estPos.bearing_deg + 180 >= 360 && estPos.bearing_deg - 180 > properHeading) { //right turn
      //set turn angle right
      turn_direction = 90;
      Serial.println("Turning Right");
      return false;
