...
void initialize()
{
//  Serial.println("Start of initialize");
...
//
//
//  Serial.println("que 5 second delay");
//  // Serial3.flush();
//  delay(5000);
...
//
//
//  Serial.println("done with checksum(protocol)");
//  // Serial3.println(protocol);
//  disable[10] = '2';
//  common::checksum(disable);
//  Serial.println("done with checksum(disable)");
//  // Serial3.println(disable);   // no GSA
//  for(int i = 10; i >= 0; i--)
//  {
//  	delay(1000);
//  	Serial.println("Start of AcquireGPRMC");
//  }
...
void loop()
{
	Serial.println("\n\nTime since last loop: " + String(millis() - lastLoopTime)+"\n\n");
	lastLoopTime = millis();
...
//   Serial.println(String(estimated_position.east_mm) + "estimated_position.east_mm");
  ...
  ParseStateError r = ParseStateError::success;//ps.update();
  data.speed_cmPs = 100;
...
//    data.speed_cmPs = C2_Results.speed_cmPs;
	...
	oldPos.x_Pos = 0;
	oldPos.y_Pos = 0;
	oldPos.bearing_deg = 0;
	oldPos.time_ms = 0;
	oldPos.speed_cmPs = 0;
	oldPos.distance_mm = 0;
	oldPos.sigma_mm = 0;
